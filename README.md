...
- **GUI:** `medical-coder-llm-gui-<os>-<arch>` (on Windows, `.exe`)
...
```bash
chmod +x medical-coder-llm-macos-arm64 medical-coder-llm-gui-macos-arm64
./medical-coder-llm-macos-arm64
```

On Intel Macs, use the `macos-x64` filenames instead. Start the web server with `./medical-coder-llm-gui-macos-<arch>` (in another terminal if you also need the CLI).

If macOS blocks the downloaded binaries, you can clear the quarantine attribute: `xattr -dr com.apple.quarantine ./medical-coder-llm-macos-* ./medical-coder-llm-gui-macos-*`

**Linux**

```bash
chmod +x medical-coder-llm-linux-x64 medical-coder-llm-gui-linux-x64
./medical-coder-llm-linux-x64
```

On ARM Linux, use the `linux-arm64` filenames. Run `./medical-coder-llm-gui-linux-<arch>` for the web UI.
...
Use the `windows-arm64` filenames on ARM Windows if you downloaded those assets. Run `.\medical-coder-llm-gui-windows-<arch>.exe` for the web UI.

### Build locally

Build two one-file binaries locally (CLI + GUI):
